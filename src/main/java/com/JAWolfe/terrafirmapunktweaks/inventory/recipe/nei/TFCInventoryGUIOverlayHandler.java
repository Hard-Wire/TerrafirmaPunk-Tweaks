package com.JAWolfe.terrafirmapunktweaks.inventory.recipe.nei;
...
public class TFCInventoryGUIOverlayHandler  implements IOverlayHandler
{
    ...
    public TFCInventoryGUIOverlayHandler(int x, int y)
    {
        offsetx = x;
        offsety = y;
    }
    
    public TFCInventoryGUIOverlayHandler()
    {
        this(57, 11);
    }
    ...
    {        
        for(IngredientDistribution distrib : assignedIngredients)
        {
            ...
            int destSlotIndex = 0;
            if(distrib.slots.length == 0) continue; //Ignore Slot - Used for 2x2 mode as NEI thinks this is a 3x3
    ...
    protected boolean canMoveFrom(Slot slot, GuiContainer gui) {
        return slot.inventory instanceof InventoryPlayer;
    }

    @SuppressWarnings("unchecked")
    protected Slot[][] mapIngredSlots(GuiContainer gui, List<PositionedStack> ingredients)
    {
    ...
    protected DistributedIngred findIngred(List<DistributedIngred> ingredStacks, ItemStack pstack)
    {
